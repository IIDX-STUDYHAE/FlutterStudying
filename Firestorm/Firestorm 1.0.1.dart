//노브 보정을 넣으려면 로직을 좀 더 많이 추가해야 하는데, 이게 메모리도 너무너무너무 많이 사용할 예정이라 하드한 보정 없이 소프트하게 보정과 비슷한 효과를 할 방법을 고안하고 있습니다... 코딩체급이 딸려서 많이 힘드네용
...
class GameColors {
  static const Color leftKnobColor = Color(0xFFADFF2F); // 왼쪽 노브 선
  static const Color leftIndicatorColor = Color(0xFFADFF2F); // 왼쪽 인디케이터+왼쪽 판정 불빛
  static const Color rightKnobColor = Color(0xFF00A1F4); // 오른쪽 노브 선
  static const Color rightIndicatorColor = Color(0xFF00A1F4); // 오른쪽 인디케이터+오른쪽 판정 불빛
}
...
class _KnobPracticeGameState extends State<KnobPracticeGame> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    directionTimer = Timer.periodic(Duration(milliseconds: 300), (timer) {
     ...
     // 0.01초마다 점수 판정, 예외처리 일단 생략. 추후에 무조건 추가해야함. 예외처리 logic이 계속 꼬임..
    scoreTimer = Timer.periodic(Duration(milliseconds: 10), (timer) {
      setState(() {

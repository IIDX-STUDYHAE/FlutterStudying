//근래 계속 제 뇌 한 구석에서 백그라운드 프로세스로 '보정 어캐함???'이 돌아가고 있었는데 며칠만에 좀 해답을 찾았습니다
//20ms마다 점수를 줄지말지 판단할 때 '방향'을 계산해보고(방향계산은 20ms 이전에 비해서 값이 음의 방향으로 변했는지 양의 방향으로 변했는지 아니면 아예 안 변했는지 판단하였습니다) 이 결과값들을 크기가 5인 리스트에 넣었습니다
//그 후 방향이 같고 위치도 맞으면 20ms마다 인디케이터 x좌표 = 노브 x좌표; 해줬습니다. 그리고 이렇게 20ms마다 해주면 떨기가 너무 쉬워져서, 좌표 간의 차이가 30일 때만 보정이 되고, 30~60일 때는 점수만 주고 보정은 안 주는 식으로 세팅함
//이렇게 left hand X right hand의 모드가 완료됐고, 이 다음 1.2.0은 노브 1개일 때, 노브 2개일 때, 노브 3개일 때를 넣는 방식으로 대격변을 좀 일으켜줄 것입니다. 
//'이미 짜여진 로직'기반으로 응용은 AI가 엄청나게 잘 하니까 잘 하면 오늘 안에 2.0.0도 올리겠습니다 감사합니다. 
